export default class News {
  renderNews(elemId, news) {
    let elem = elemId;
    if (elem && news.status == 'ok') {
      elem.innerHTML = news.articles.reduce((txt, li) => txt + `<li class='col-xs-12 col-sm-12 col-lg-6 col-xl-6'>${this.render(li)}</li>`, `<ul class="articles row">`) + `</ul>`;
    }
  }
